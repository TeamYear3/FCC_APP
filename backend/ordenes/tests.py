...
from .use_cases import enviar_email_orden_background, CrearOrdenTrabajoUseCase
...
class OrdenTrabajoEmailTest(TestCase):
    ...
    @patch("ordenes.use_cases.enviar_email_orden_background")
    def test_crear_orden_use_case_dispara_hilo_email(self, mock_enviar_email):
        """Verifica que CrearOrdenTrabajoUseCase inicia el proceso de envío de email."""
    ...
    def test_crear_orden_use_case_tolerancia_a_errores_de_hilo(self, mock_thread):
        """Verifica que si ocurre un error al lanzar el hilo, la creación de la orden no falla."""
        # Configurar el mock para que falle al llamar a start()
        ...
        # La orden debe crearse a pesar del fallo
        self.assertIsNotNone(orden.id)
