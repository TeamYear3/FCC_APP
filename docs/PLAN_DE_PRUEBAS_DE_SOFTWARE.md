...
25 casos de prueba funcionales organizados en 5 módulos (5 casos por módulo) con la estructura reglamentaria de 10 atributos:
...
##### Módulo 5: Funcionalidades Propias del Taller (Turnos, Diagnóstico y Facturación ARCA)

| ID Caso | Módulo | Descripción del Caso | Precondiciones | Pasos de Ejecución | Resultado Esperado | Prioridad | Resultado Real | Estado (Pass/Fail) | Evidencia / Notas |
|---|---|---|---|---|---|:---:|---|:---:|---|
| **TEST-P-001** | Taller/Turnos | Agendamiento de turnos para servicio mecánico | Usuario autenticado; calendario con slots disponibles. | 1. Navegar a `/admin/turnos`.<br>2. Seleccionar fecha y box libre.<br>3. Asignar cliente, vehículo y mecánico.<br>4. Clic en "Guardar Reserva". | Se crea el turno en base de datos (`HTTP 201 Created`), se bloquea el slot en el calendario y se envía confirmación por email. | **Crítica** | *Pendiente* | **Pendiente** | Captura del calendario FullCalendar con el turno agendado y confirmación en base de datos. |
| **TEST-P-002** | Taller/Diagnóstico | Inspección fotográfica de daños y diagnóstico inicial | OT activa en estado `RECIBIDO` o `EN_DIAGNOSTICO`. | 1. Abrir modal de diagnóstico en la OT.<br>2. Adjuntar fotografías del vehículo (JPG/PNG).<br>3. Registrar observaciones técnicas.<br>4. Guardar inspección. | El backend procesa las imágenes, las aloja en el almacenamiento Cloudinary y asocia las URLs al diagnóstico de la OT. | **Alta** | *Pendiente* | **Pendiente** | Captura de galería fotográfica en la ficha de la OT y respuesta HTTP 201 con URLs seguras. |
| **TEST-P-003** | Taller/Facturación | Emisión de factura electrónica con CAE mock de ARCA | OT en estado `FINALIZADO` con presupuesto aprobado. | 1. Navegar a la sección de facturación de la OT.<br>2. Seleccionar tipo de comprobante (Factura B).<br>3. Clic en "Emitir Factura Electrónica". | El servicio invoca el mock de ARCA, obtiene el CAE (`74321890123456`) con vencimiento a 10 días, emite la factura y habilita la descarga en PDF. | **Crítica** | *Pendiente* | **Pendiente** | Captura del comprobante emitido con número fiscal, CAE asignado y PDF descargable. |
| **TEST-P-004** | Taller/Portal Cliente | Consulta pública y aprobación digital de presupuesto | OT en estado `PRESUPUESTADO` con token de consulta emitido. | 1. Acceder al portal cliente mediante enlace con token.<br>2. Visualizar detalle de ítems, repuestos y mano de obra.<br>3. Clic en "Aprobar Presupuesto". | La API valida el token, transiciona la OT de `PRESUPUESTADO` a `APROBADO`, emite alerta WebSocket al taller y bloquea modificaciones en los ítems. | **Crítica** | *Pendiente* | **Pendiente** | Captura de la vista cliente con confirmación de aprobación y cambio de estado verificado en el Kanban del taller. |
| **TEST-P-005** | Taller/Turnos | Control de sobrecupo y restricción de reserva en boxes | Boxes del taller ocupados al 100% en la fecha seleccionada. | 1. Intentar agendar un nuevo turno en la fecha saturada.<br>2. Completar formulario de turno.<br>3. Enviar solicitud. | El backend rechaza la solicitud (`HTTP 400 Bad Request`) informando capacidad colmada; la UI resalta el día como no disponible. | **Alta** | *Pendiente* | **Pendiente** | Captura de mensaje de alerta en frontend y código de estado 400 en consola de red. |
